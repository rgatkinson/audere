...
export default class Text extends React.Component<Props> {
  _oneReplace(str: string, bold: boolean, contentKey: string) {
    return str.split("①").map(
      (subStr, j) =>
        j % 2 === 1 ? (
          <Svg key={"circle1" + j} height={REGULAR_TEXT} width={REGULAR_TEXT}>
            <Svg.Circle
              cx={REGULAR_TEXT / 2}
              cy={REGULAR_TEXT / 2}
              r={REGULAR_TEXT / 2}
  ...
  _twoReplace(str: string, bold: boolean, contentKey: string) {
    return str.split("②").map(
      (subStr, j) =>
        j % 2 === 1 ? (
          <Svg key={"circle2" + j} height={REGULAR_TEXT} width={REGULAR_TEXT}>
            <Svg.Circle
              cx={REGULAR_TEXT / 2}
              cy={REGULAR_TEXT / 2}
              r={REGULAR_TEXT / 2}
  ...
  _threeReplace(str: string, bold: boolean, contentKey: string) {
    return str.split("③").map(
      (subStr, j) =>
        j % 2 === 1 ? (
          <Svg key={"circle3" + j} height={REGULAR_TEXT} width={REGULAR_TEXT}>
            <Svg.Circle
              cx={REGULAR_TEXT / 2}
              cy={REGULAR_TEXT / 2}
              r={REGULAR_TEXT / 2}
