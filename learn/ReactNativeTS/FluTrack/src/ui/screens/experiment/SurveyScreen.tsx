...
export default class ScrollScreen extends React.Component<Props> {
  ...
  _addData = () => {
    ...
    questions[sectionExists ? sectionIndex : questions.length] =
      {title: item.section,  data: [...sectionData, item.data] };
    this.setState({ questions, currentItemIndex: this.state.currentItemIndex + 1});
  }

  _removeData = () => {
    const currentSectionIndex = this.state.questions.length - 1;
    const currentSectionItemIndex = this.state.questions[currentSectionIndex].data.length - 1;
      ...
      this.setState({
        questions: [
          ...this.state.questions.slice(0, currentSectionIndex), {
            ...
            data: this.state.questions[currentSectionIndex].data.slice(0, currentSectionItemIndex),
          },
        ],
        currentItemIndex: this.state.currentItemIndex - 1,
      });
      ...
      <View style={styles.container}>
        ...
        <SectionList
          ...
          renderItem={({ item, index, section }) => {
            const activeSection =
              this.state.questions[this.state.questions.length - 1].title === section.title;
