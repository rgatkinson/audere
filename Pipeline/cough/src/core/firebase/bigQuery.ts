...
export class BigQueryTableImporter {
  ...
  constructor(project: string, dataset: string, email: string, key: string) {
  ...
  public async listTables(): Promise<string[]> {
    const response = await this.client.dataset(this.dataset).getTables();

    const tables = response[0];
    logger.info(`Listed ${tables.length} tables within ${this.dataset}`);
