...
import { BigQueryTableImporter } from "./bigQuery";
import { AnalyticsModels } from "./models";
...
export class FirebaseImport {
  ...
  private readonly models: AnalyticsModels;
  ...
  public async findTablesToUpdate(): Promise<Map<string, number>> {
    ...
    const existingVersions = await this.models.analyticsTables.findAll({
      where: {
  ...
  public async importAnalytics(tableList: Map<string, number>) {
    ...
    for (let i = 0; i < tables.length; i++) {
      ...
      await this.sql.transaction(async t => {
        ...
        await this.models.analytics.destroy({
          where: {
        ...
        do {
          ...
          await this.models.analytics.bulkCreate(rows, {
            transaction: t,
          });
        } while (token != null);
        ...
        await this.models.analyticsTables.upsert(
          {
            name: name,
